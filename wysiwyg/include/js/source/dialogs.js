...
function XK_MakeAnchorSelect(id)
{
	var Anchors=new Array();
	document.getElementById('anchor').options[0]= new Option('--','');
	Anchors=window.opener.getAnchors(id);
	if (Anchors.length>0)
	for (i=0;i<Anchors.length;i++)
	{
		text=(Anchors[i]);
		text=text.replace(/<a name=\"(.*?)\"><\/a>/gi,'$1');
		document.getElementById('anchor').options[i+1] = new Option(text,'#'+text);
	}
...
function tablePreview() 
{
	...
	previewItem.style.borderCollapse = "collapse";

};	

function XK_TableC(n, id, color)
{
	textfield=document.getElementById('coloroption').value;
	document.getElementById(textfield).value=color;
	XK_showHideDiv(id,'colordiv');
	tablePreview();
};
...
{
	var cell					= 	new Object();
	cell["borderLeftStyle"]		=	document.getElementById('borderLeftStyle').value;
	cell["borderRightStyle"]	=	document.getElementById('borderRightStyle').value;
	cell["borderTopStyle"]		=	document.getElementById('borderTopStyle').value;
	cell["borderBottomStyle"]	=	document.getElementById('borderBottomStyle').value;
...
function XK_ImgPrev(n, id, color)
{
	textfield=document.getElementById('coloroption').value;
	document.getElementById(textfield).value=color;
	XK_showHideDiv(id,'colordiv');
