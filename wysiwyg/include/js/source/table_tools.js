...
function XK_useTableOps(Option,id)
{
	...
	else
	{
		...
		XK_showHideDiv(id,'tpropbutton','TableOps');
	}
...
function XK_CellColor(id, name, color)
{
  	myTableNode = new XK_tableNode(id);
	if (myTableNode.td==null)return;
...
function XK_createTable(id,table)
{
	...
	myTable.widthUnits = table.width_units;
	myTable.height = table.height;
	myTable.heightUnits = table.height_units;
	myTable.createTable();
};
...
XK_table.prototype.createTable = function() 
{
	...
	else XK_InsertWithoutFormat(this.id,table);
    }
};
...
{
	//if number of rows=1 don't do anything under Mozilla and remove table under iexplore
...
XK_tableNode.prototype.getTd = function()
{
	...
	{
		var td = this.IsInsideThisTagMoz ('TD');
		if (td==false)td = this.IsInsideThisTagMoz ('TH');
...
XK_tableNode.prototype.getTable = function()
{
	...
	{
		var table = this.IsInsideThisTagMoz ('TABLE');
...
XK_tableNode.prototype.IsInsideThisTagMoz = function(tagname)
{
	var IframeId='iframe'+this.id;
...
function XK_tableClick(id,i,j) 
{
	XK_Easytable(id,i,j);
	XK_tableOut(id);
	XK_showHideDiv(id,'tablebutton','tablepicker');
};
...
function XK_tableProp(id,ntProps)
{
   ...
   {
      // set new settings
      if (ntProps.width)tTable.style.width = ntProps.width;
	  else tTable.style.width ='';
	  tTable.removeAttribute('width',0);

      if (ntProps.height)tTable.style.height = ntProps.height;
	  ...
	  if(isie)
	  {
	  	if (ntProps.borderColor)
	  	{
	  		tTable.borderColor = ntProps.borderColor;
	  ...
	  else
	  {
	  	if (ntProps.borderColor)
	  	{
	  		tTable.setAttribute('borderColor',ntProps.borderColor);
	  		tTable.style.borderCollapse = "collapse";
	  	}
	  	else
	  	{ 
	  		tTable.removeAttribute('borderColor',0);
	  		tTable.style.borderCollapse = "";
	  	}
	  }	
...
function XK_toggleBorders(id, root)
{       
    var IframeId='iframe'+id;
    root=document.getElementById(IframeId).contentWindow.document.body;
    var showBorders=document.getElementById('borderstoggle'+id);
    ...
    {
		if (showBorders.value == 'off')
		{
			tbls[ti].border=(tbls[ti].border)-(1-2);
      		b=1;
		...
		{
			if (!tbls[ti].border==0)
			tbls[ti].border=tbls[ti].border-1;
			b=0;
		}       
    }
    if(b==0)showBorders.value='off';
	else showBorders.value='on';
	return;
};